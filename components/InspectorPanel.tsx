...
const mergeNodeConfig = (base: NodeConfig | undefined, patch: Partial<NodeConfig>): NodeConfig => {
  if (!base) {
  ...
  return (
    <div className="flex h-full w-80 flex-col overflow-y-auto border-l border-border bg-card">
      <div className="sticky top-0 border-b border-border bg-card p-4">
        ...
        <p className={`mt-1 text-xs font-medium ${getNodeTypeColor(selectedNode.data.type)}`}>
      ...
      <ScrollArea className="flex-1 space-y-6 overflow-y-auto p-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        ...
        {selectedNode.data.type === 'automation' && blockId?.includes('sla') && (
          <Card className="p-4 mb-4">
            ...
            <Input
              type="number"
              value={config?.nodeType === 'automation' ? config.duration ?? 60 : 60}
              onChange={(event) =>
                handleConfigChange({
                  nodeType: 'automation',
